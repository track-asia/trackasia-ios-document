...
* Fix: If the directions API endpoint doesn't include audio instructions, `didArrive:` would never be called.
  * Merged in <https://github.com/track-asia/trackasia-navigation-ios/pull/72>
* Updated "turf" geometry library from 0.2.2 to 2.8.0
    * Merged in https://github.com/track-asia/trackasia-navigation-ios/pull/91
* Only require background audio when using speech synthesis in https://github.com/track-asia/trackasia-navigation-ios/pull/64
* Fix: Respond to changes in dynamic type without having to restart the app in https://github.com/track-asia/trackasia-navigation-ios/pull/65
* Fix: crash in EndOfRouteViewController and restore its presentation by in https://github.com/track-asia/trackasia-navigation-ios/pull/71
...
* Updated TrackAsia Native dependency to ios-v6.0.0 (https://github.com/track-asia/trackasia-native/releases/tag/ios-v6.0.0). Implementers need to change the prefix MLN to MLN for all TrackAsia Native classes that are referenced.
...
* Removed implicit default dependencies on MapBox tileservers by requiring explicit styles URLs in more places.
    - Merged in <https://github.com/track-asia/trackasia-navigation-ios/pull/45>.
    ...
    - Added `Day/NightStyle(demoStyle: ())` as an explicit alternative when the user doesn't have a tileserver handy. This uses TrackAsia's demo style and is intended for testing and demonstration use only.
    - Deprecated `DayStyle()`/`NightStyle()` initializers because they were backed by an implicit tile service. If these default styles *are* still used, they'll now use the TrackAsia demo style.
    - `NavigationViewController` now expects explicit style URLs with `NavigationViewController(route:dayStyleURL:nightStyleURL:...)` or NavigationViewController(route:dayStyle:nightStyle:...)` and the existing initializer, which allowed "default" styles, is deprecated and uses the TrackAsia demo styles.
* Fix: NavigationViewController was not re-routing when the user went off route.
    - Merged in <https://github.com/track-asia/trackasia-navigation-ios/pull/47>
* Fix: NavigationViewController displayed incorrect `speedMultiplier` when using SimulatedLocationManager
    - Merged in <https://github.com/track-asia/trackasia-navigation-ios/pull/49>
* Adjusted the camera during navigation to show more of what's "ahead" in the route - effectively moving the puck lower on the screen. As before, see `NavigationMapViewCourseTrackingDelegate.updateCamera(_:location:,routeProgress:)` if you want to customize this behavior.
    - Merged in <https://github.com/track-asia/trackasia-navigation-ios/pull/92>
